...
def close_case(case_id: int) -> dict:
    """Close a case in DFIR-IRIS.

    Args:
        case_id: The numeric case ID to close
    """
    try:
        return _post(f"/manage/cases/{case_id}/close")
    except Exception as e:
        return _err(e)


@mcp.tool()
def update_case_status(case_id: int, status_id: int) -> dict:
    """Update the status of a case.

    Args:
        case_id: The numeric case ID
        status_id: Status ID (0=Open, 1=In progress, 2=Closed, 3=On hold)
    """
    try:
        return _post(
            f"/manage/cases/{case_id}/status/update",
...
@mcp.tool()
def get_notes(case_id: int) -> dict:
    """List all notes with their content for a case.

    Args:
        case_id: The numeric case ID
    """
    try:
        return _get("/case/notes/list", {"cid": case_id})
    except Exception as e:
        return _err(e)
